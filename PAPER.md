...
The classifier component is deliberately swappable: the implementation is built on Pydantic AI's per-provider model classes, so switching backend — Mistral, Together, OpenRouter, a local vLLM server, or any OpenAI-compatible endpoint — is a one-line change. This is what made the cross-paradigm validation in §6.2 cheap to run. Future revisions of the corpus can use a different classifier without changing any other code.
